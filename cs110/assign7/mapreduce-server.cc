...
MapReduceServer::MapReduceServer(int argc, char *argv[]) throw (MapReduceServerException) 
  : user(getUser()), host(getHost()), cwd(getCurrentWorkingDirectory()),
    serverPort(computeDefaultPortForUser()), verbose(true), mapOnly(false), noReduce(false), serverIsRunning(false) {
...
void MapReduceServer::orchestrateWorkers() throw () {
  ...
  while (true) {
    ...
    pool.schedule([this, clientSocket, clientIPAddress] {
...
void MapReduceServer::spawnMappers() throw() {
  ...
  for (size_t i = 0; i < numMappers; i++) {
    string command = buildCommand(mapperNodes[i % mapperNodes.size()], executablePath, mapperExecutable, mapOutputPath);
    string node = mapperNodes[i % mapperNodes.size()];

    pool.schedule([this, command, node] {
      spawnWorker(node, command);
      });
  }
  pool.wait();
...
void MapReduceServer::spawnReducers() throw() {
  ...
  for (size_t i = 0; i < numMappers; i++) {
    string command = buildCommand(reducerNodes[i % reducerNodes.size()], executablePath, reducerExecutable, reduceOutputPath);
    string node = reducerNodes[i % reducerNodes.size()];
...
string MapReduceServer::buildCommand(const string& workerHost, const string& executablePath, 
  ...
  ostringstream oss;
  oss << "ssh -o ConnectTimeout=5 " << user << "@" << workerHost
...
void MapReduceServer::groupByKey() throw() {
  ...
  stageFiles(mapOutputPath, outFiles);
  outFiles.sort();

  for (size_t i = 0; i < numReducers; i++) {
    string path = reduceInputPath + "/" + to_string(i)  + ".grouped";
    fds.emplace_back(path);
    filenames.push_back(path);
  }
  
  for (auto name : outFiles) {
    ifstream fs(name);
    string line;
    // TODO: figure out why hashing is wonky
    while (getline(fs, line)) {
      string key = line.substr(0, line.find(" "));
      fds[hash<string>()(key) % numReducers] << line << endl;
    }
  }

  for (auto name : filenames) {
    string sortCmd = "sort -o '" + name + "' '" + name + "'";
    system(sortCmd.c_str());

    string tmpFilename = name + ".tmp";
    string groupCmd = "./group-by-key.py <'" + name + "' >'" + tmpFilename + "' && "
                      + "mv '" + tmpFilename + "' '" + name + "'";
