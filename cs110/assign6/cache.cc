...
void HTTPCache::lockRequestMutex(const HTTPRequest &request) {
    size_t bucket = hashRequest(request) % kNumMutexes;
    cacheMutex[bucket].lock();
}

void HTTPCache::unlockRequestMutex(const HTTPRequest &request) {
    size_t bucket = hashRequest(request) % kNumMutexes;
    cacheMutex[bucket].unlock();
}
