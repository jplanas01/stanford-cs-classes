 ...
 */
  ...
  /* Lock and unlock the mutex associated with a particular request to prevent
   * race conditions. Identical requests should hash to the same bucket.
   */
  void lockRequestMutex(const HTTPRequest& request);
  void unlockRequestMutex(const HTTPRequest &request);
 ...
 private:
  ...
  std::mutex cacheMutex[kNumMutexes];
};
