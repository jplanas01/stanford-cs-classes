/* recursionproblems-extra.cpp
 * ---------------------
 * Solves a number of problems using recursion. Now with extensions.
 *
 * Extensions:
 *	-Robust isBalanced (line 122)
 *	-Robust grammar solver (ignores whitespace) (line 240)
 *	-Changed color picker to use user-chosen arbitrary hex color specification
 *		(ie, inputting 000000 results in black.) (line 318 in recursionmain.cpp)
 */
 ...
 */
bool isBalancedRecur(string exp) {
	if (exp == "") {
		return true;
	}
	...
	if (loc != -1) {
		string newExp = exp;
		newExp.erase(loc, 2);
		return isBalancedRecur(newExp);
	}
	return false;
}

/* Sanitizes the string before passing it to the recursive solver. */
bool isBalanced(string exp) {
	const string to_match = "()[]{}<>";
	for (int i = 0; i < exp.size(); i++) {
		if (to_match.find(exp[i]) == -1) {
			exp.erase(i, 1);
			i--;
		}
	}
	return isBalancedRecur(exp);
}
...
/* Removes superfluous whitespace from each string in a vector. */
void sanitizeVector(Vector<string> &vec) {
...
void buildGrammarMap(istream &input, Map<string, Vector<string> > &map) {
	...
	while (getline(input, line)) {
		...
		sanitizeVector(terminals);
		sanitizeVector(lineParts);
