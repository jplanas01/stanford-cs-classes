...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for i in xrange(1, len(word)+1):
        for let in string.ascii_lowercase:
            corrupt_letters = word[i-1]
            correct_letters = corrupt_letters + let
            correction = word[1:i] + let + word[i:]
            ret.append(Edit(correction, corrupt_letters, correct_letters))
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(0, len(word)-2):
        corrupt_letters = word[i+1:i+3]

        # Can't really have a transpose of switching two of the same letter
        # around (edit distance is 0 between these words)
        if corrupt_letters[0] == corrupt_letters[1]:
            continue

        correct_letters = corrupt_letters[1] + corrupt_letters[0]
        correction = list(word)[1:]
        correction[i], correction[i+1] = correction[i+1], correction[i]
        ret.append(Edit(''.join(correction), corrupt_letters, correct_letters))
    return ret
  ...
  def replaceEdits(self, word):
    ...
    for i in xrange(0, len(word)):
        for let in string.ascii_lowercase:
            # Doesn't make sense to replace a letter with itself  
            if word[i] == let:
                continue

            correction = word[:i] + let + word[i+1:]
            ret.append(Edit(correction, word[i], let))
